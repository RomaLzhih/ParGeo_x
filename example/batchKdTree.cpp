...
bool
is_knn( const TestType& t )
{
   return ( t == KNN ) || ( t == KNN2 ) || ( t == KNN3 ) || ( t == DUAL_KNN );
}

static std::ostream&
operator<<( std::ostream& os, const TestType& t )
{
   switch( t )
   {
   case INVALID:
...
void
timeConstruction( parlay::sequence<point<dim>> const& P, int rounds,
                  int percentage )
{
...
template <int dim, class Tree>
void
timeContainsQuery( parlay::sequence<point<dim>> const& P, int rounds )
{
   ...
   for( int i = 0; i < rounds; ++i )
   {
      ...
      // std::cout << "  -> " << rep << "x query-time (-) = " << query_time_1 <<
      // std::endl;
      ...
      // std::cout << "  -> " << rep << "x query-time (+) = " << query_time_2 <<
      // std::endl;
...
template <int dim, class Tree>
void
timeRangeQuery( parlay::sequence<point<dim>> const& P, int rounds )
{
   ...
   double x[dim];
   for( int i = 0; i < dim; i++ ) x[i] = pMin[i] / 10;
   auto qMin = point<dim>( x );
   for( int i = 0; i < dim; i++ ) x[i] = pMax[i] / 10;
   ...
   for( int i = 0; i < rounds; ++i )
   {
      ...
      // std::cout << "  -> " << rep << "x query-time (" << p.size() << "
      // points) = " <<time
      //<< std::endl;
      if( i > 0 ) std::cout << ", ";
      std::cout << time;
...
template <int dim, class Tree>
void
timeKnn( parlay::sequence<point<dim>>& P, int rounds, int k, int k_type,
         const TestType& type )
{
   // constexpr int rep = 1;
...
template <int dim, class Tree>
void
timeInsert( parlay::sequence<point<dim>> const& P, int rounds )
{
...
template <int dim, class Tree>
void
timeDelete( parlay::sequence<point<dim>> const& P, int rounds, int percentage )
{
   if( percentage == -1 ) percentage = 10;  // default

   constexpr int divisor = 10;
   if( percentage % divisor != 0 )
      throw std::runtime_error( "delete setup error!" );
   ...
   for( int i = 0; i < rounds; ++i )
   {
      ...
      std::cout << "TOTAL SEARCH TIME: " << tree.total_search_time << " s"
                << std::endl;
      std::cout << "TOTAL remove TIME: " << tree.total_bbox_time << " s"
                << std::endl;
      std::cout << "TOTAL LEAF TIME: " << tree.total_leaf_time << " s"
                << std::endl;
#endif
      std::cout << time;
      t.stop();
...
template <int dim, class Tree>
void
dispatchTest( parlay::sequence<point<dim>>& P, const TestOptions& test_options )
{
   std::cout << "[";
   ...
   case CONSTRUCTION:
   {
      timeConstruction<dim, Tree>( P, test_options.rounds,
                                   test_options.percentage );
      break;
   }
   case CONTAINS:
   {
   ...
   case KNN:
   {
      timeKnn<dim, Tree>( P, test_options.rounds, test_options.k,
                          test_options.k_type, test_options.type );
      break;
   }
   case DELETE:
   {
...
static bool
UseCO( const TestType& t )
{
   return ( t != INSERT ) && ( t != INSERT_DELETE ) && ( t != KNN2 ) &&
          ( t != KNN3 );
}
static bool
UseBHL( const TestType& t )
{
   return ( t != KNN2 ) && ( t != KNN3 );
}

template <int dim, bool parallel, bool coarsen>
void
timeTrees( parlay::sequence<point<dim>>& P, const TestOptions& test_options )
{
   ...
   typedef LogTree<NUM_TREES, BUFFER_LOG2_SIZE, dim, point<dim>, parallel,
                   coarsen>
       LogTree_t;

   std::string thread_str =
       ( parallel
             ? ( " (" + std::to_string( parlay::num_workers() ) + " threads)" )
             : ( " (serial)" ) );
   ...
   {
      std::cout << " - Timing Binary Heap Layout Tree" << thread_str
                << std::endl;
...
template <int dim>
inline void
runTests( const char* iFile, const TestOptions& test_options )
{
   // parlay::sequence<point<dim>> Points =
   // readPointsFromFile<point<dim>>(iFile);
   parlay::sequence<point<dim>> Points =
       pargeo::pointIO::readPointsFromFile<point<dim>>( iFile );
   std::cout << "Timing " << test_options.type << " (dim = " << dim
             << ", #points = " << Points.size()
             << ( ( is_knn( test_options.type ) )
                      ? ( ", k=" + std::to_string( test_options.k ) )
                      : "" )
             << ")" << std::endl;
...
TestOptions
parseCmdLine( pargeo::commandLine& P )
{
   // parse test options
   ...
   {
      test_options.run_serial = test_options.run_parallel =
          true;  // neither set, so default to both
   }

   test_options.run_co = P.getOption( "--co" );
   test_options.run_bhl = P.getOption( "--bhl" );
   test_options.run_log = P.getOption( "--log" );
   if( !( test_options.run_co || test_options.run_bhl ||
          test_options.run_log ) )
   {
      // none set, so default to all
      test_options.run_co = test_options.run_bhl = test_options.run_log = true;
   }

   auto type_str = P.getOptionValue(
       "--type", "construction" );  // default to construction times
...
main( int argc, char* argv[] )
{
   pargeo::commandLine P(
       ...
       "--log] [--type "
       "construction|range|contains|knn|knn2|knn3|dual_knn|delete|insert|"
       "insert_delete] [-k "
